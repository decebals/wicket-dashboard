...
import com.asf.wicket.dashboard.DashboardUtils;
import com.asf.wicket.dashboard.Widget;
import com.asf.wicket.dashboard.WidgetLocation;
...
class DashboardColumnPanel extends GenericPanel<DashboardColumn> {
	...
	public DashboardColumnPanel(String id, IModel<DashboardColumn> dashboardColumn) {
		super(id, dashboardColumn);
		
		final int columnIndex = getModelObject().getIndex();
		...
		final List<Widget> widgets = getModelObject().getWidgets();
...
//				if (widget.isCollapsed()) {
					WidgetPanel widgetPanel = createWidgetPanel("widget", new WidgetModel(widget.getId()));					
